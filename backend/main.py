...
from dotenv import load_dotenv # NEW: Load env vars
...
# Reads from Environment or falls back to defaults (Safe for Deploy)
SECRET_KEY = os.getenv("SECRET_KEY", "aegis_top_secret_key_change_this_in_production")
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIRE_MINUTES = 60
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="token")

# AUDIT DB: Uses Env Var. Fallback to local for testing.
AEGIS_DB_DSN = os.getenv("DATABASE_URL", "postgresql://postgres:root@localhost:5432/aegis_db")
...
# --- LIFECYCLE: CONNECTION POOLING ---
@app.on_event("startup")
async def startup():
    # Create a pool of connections to the Audit DB
    # min_size=1, max_size=10 ensures we don't overload the database
    try:
        app.state.audit_pool = await asyncpg.create_pool(dsn=AEGIS_DB_DSN, min_size=1, max_size=10)
        print("[SYSTEM] Audit Database Connection Pool Established")
    except Exception as e:
        print(f"[CRITICAL] Failed to connect to Audit DB: {e}")
...
async def shutdown():
    ...
    if hasattr(app.state, 'audit_pool'):
        ...
        print("[SYSTEM] Connection Pool Closed")
...
def create_certificate(db_name, table_name, record_id, original_data, columns_cleaned, timestamp, salt, admin_user):
    try:
        ...
        pdf.cell(190, 5, txt="Generated by Aegis Sovereignty Engine v3.0 (Optimized Core)", ln=1, align='C')
        ...
        pdf.multi_cell(190, 6, txt="This document certifies that PII has been irreversibly pseudonymized or masked.")
    ...
    try:
        ...
        try:
            ...
            for r in rows:
                ...
                if any(k in col for k in SAFE_KEYWORDS): strategy = "PRESERVE"; reason="Safe Keyword"
                elif 'email' in col: strategy = "EMAIL_MASK"; reason = "Email Detected"
                elif 'card' in col or 'ssn' in col or 'phone' in col: strategy = "MASK"; reason = "Sensitive ID"
                elif any(k in col for k in PII_KEYWORDS) and dtype in ['text', 'character varying']: strategy = "HASH"; reason = "Generic PII"
...
async def execute_erasure(req: BulkErasureRequest, background_tasks: BackgroundTasks, current_user: str = Depends(get_current_user)):
    ...
    # 1. CONNECT TO TARGET (Unpooled - Cloud)
    try:
        target_dsn = build_dsn(req.connection)
        ssl_ctx = get_ssl_context(req.connection.ssl_enabled)
        conn = await asyncpg.connect(target_dsn, ssl=ssl_ctx)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Target DB Connection Error: {e}")
        ...
        with zipfile.ZipFile(temp_zip_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            for target_id in req.target_ids:
                try:
                    ...
                    # OPTIMIZATION: USE POOL for Audit Log
                    # Instead of opening a new connection, we borrow one from app.state.audit_pool
                    try:
                        ...
                        print(f"[CRITICAL WARNING] Audit Log Failed: {e}")
